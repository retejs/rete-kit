...
export function getDependencyTopo<T extends PackageMeta>(list: T[]): Array<T & Topology> {
  ...
  const edges = Object.values(packages).reduce((acc, { config, dependencies }) => {
    return [ ...acc, ...dependencies.map(dependencyName => [dependencyName, config.name] as [string, string])]
  }, [] as [string, string][])
