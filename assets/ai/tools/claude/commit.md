# Claude Code commit — finalize setup
...
Apply from the page: expected paths for `CLAUDE.md` / `.claude/CLAUDE.md`, `@AGENTS.md` import, and how to verify project instructions load at session start.
...
Apply from the page: skill layout, frontmatter requirements, where installed skills should appear (`/skills`, `/skill-name`).
...
- [ ] **Project instructions** — Rete.js reference content from `.rete/ai/AGENTS.md` is merged (not a blind copy; no duplicate sections). Either merged into `AGENTS.md` with `CLAUDE.md` importing `@AGENTS.md`, or merged into root `CLAUDE.md` when `AGENTS.md` is absent. Location matches [Memory — CLAUDE.md](https://code.claude.com/docs/en/memory#claudemd-files).
- [ ] **Skills** — every directory under `.rete/ai/skills/` is installed under `.claude/skills/` per [Skills](https://code.claude.com/docs/en/skills) (including reference files); valid frontmatter; visible via `/skills` or skill list in Claude Code.
- [ ] **Subagents** — every file under `.rete/ai/agents/` is installed as `.claude/agents/<name>.md` per [Subagents](https://code.claude.com/docs/en/sub-agents); `readonly: true` mapped to `tools` (`rete-package-reviewer`: read-only; `rete-code-investigator`: includes `Bash` for `source-context`); descriptions specific enough to route tasks.
...
3. Re-run setup (`rete-kit ai --tool claude`) if needed.
...
Use **one** `AskUserQuestion` call. **Title:** `Rete.js AI setup — optional next steps`
...
- If **yes** → skill **`rete-clone`**: propose a target directory based on step 6, confirm path with the user (`AskUserQuestion` for fixed choices; chat or **Other...** for a custom path), `git clone` only needed repos. **Never symlink** from `gh-mirror/readonly-repositories/`.
...
If `GITHUB_TOKEN` is missing or empty in `.rete/scripts/.env` and the environment, use **one** `AskUserQuestion` call before `npm run clone-org`.
...
Summarize: Claude Code install done, `.rete/ai/` removed, `.rete/scripts/` kept, whether `gh-mirror/` was created (and whether a **GitHub token** was configured), whether working copies were created (and **which path**), or what was declined. Mention **`rete-setup`** / **`rete-clone`** for later.
