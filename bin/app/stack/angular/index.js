...
class AngularBuilder {
    ...
    async putAssets(name, version, template) {
        ...
        const appModulePath = (0, path_1.join)(src, 'app', 'app.module.ts');
        const appFile = await fs_1.default.promises.readFile(appModulePath, { encoding: 'utf-8' });
        await fs_1.default.promises.writeFile(appModulePath, await template.build(appFile));
    }
