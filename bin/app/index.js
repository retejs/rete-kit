...
async function createApp({ name, stack, version, features, depsAlias, forceInstall = false, next = false }) {
    ...
    const builderVersions = builder.versions;
    const selectedVersion = version || await (0, inquirer_1.select)('Version', builderVersions.map(value => ({
        name: String(value),
        value: value
    })));
    ...
    const featuresList = [
        ...
        new Features.Vue(builder.foundation === 'vue' && !(builder instanceof stack_1.NuxtBuilder)
            ? selectedVersion
            : 3, next),
    ...
    const sources = await template_builder_1.TemplateBuilder.getTemplates();
    const jobs = Features.resolveJobs(sources, optionalFeatures, selectedStack, features
        ? { features }
        : {
            selected: await (0, inquirer_1.select)('Select features', optionalFeatures.map(feature => ({
                name: feature.name,
                value: feature
            })), true)
        });
    ...
    const templateBuilder = new template_builder_1.TemplateBuilder(Features.featureKeys(activeFeatures));
    ...
    for (const job of jobs) {
        const currentBuilder = new template_builder_1.TemplateBuilder(Features.featureKeys([
            ...mandatoryFeatures,
            ...job.features
        ]));
        try {
            ...
            console.error(e);
            (0, throw_1.throwError)(`failed to build template "${job.name}"`);
        }
    }
